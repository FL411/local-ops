...
- **启动台空列表被缓存/误写入**：`/api/state` 的进程扫描仍走 2.2 秒缓存，但每次响应都从磁盘 `config.json`（主文件不可读时才用 `.bak`）重建启动台卡片，不再返回内存或缓存里的空列表。`Config.snapshot()` / `update()` 也先读磁盘再工作，主题/设置落盘不会把磁盘上的卡片写成空列表。
