...
- **项目入口与公开说明**：页面顶栏 GitHub 按钮改为指向当前 Windows 仓库；README 首屏改为面向使用场景的项目介绍、快速开始与反馈入口，并同步优化 GitHub About 和 Topics。
...
- **残留进程自检避免误杀诊断命令**：实例扫描改用 psutil 保留参数边界的 `argv`，只把 Python 解释器直接执行本项目 `server.py` 的进程列为清理候选；PowerShell 文本和 `python -c` 代码即使提到 `server.py` 也不会被误判。
- **重复运行 EXE 后启动台偶发为空**：`LocalOpsConsole.exe` 现在先用独立探测程序判断已有实例；健康实例只打开页面，磁盘有卡片但实例内存为空时才启动自愈替换。新实例必须在绑定端口前读到启动器已确认的磁盘卡片，否则退出并由启动器自动重试一次；浏览器也只会在就绪检查通过后打开。后端不再因一次较慢的 `/api/state` 探测超时误杀健康实例，`start.bat` 同步采用相同判定。
...
- **启动台空列表被缓存/误写入**：`/api/state` 不再把「内存卡片为空但磁盘仍有配置」的快照当作成品返回；会同步从 `config.json`（主文件不可读时才用 `.bak`）把卡片读回后再响应。主题/设置等落盘若碰上这种空内存，也不会把磁盘上的卡片写成空列表。
...
- **启动后服务监控降级（psutil 只装到 user-site）**：`start.bat` 用 `python.exe` 探测 `import psutil`，用户目录安装会被当成成功；`LocalOpsConsole.exe` 则完全不安装。随后 `pythonw` 看不到 user-site，页面提示「降级运行：服务监控数据可能不完整」。现由 `launcher_check.py ensure-runtime` 按 `python -s`（与 pythonw 一致）探测，并把 `psutil>=7.2` 装进**当前解释器**（失败时 `--target` 到 realpath 后的 site-packages，兼容 uv junction）。`start.bat` 与 exe 共用这条路径。
