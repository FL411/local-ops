...
| 方式 | 操作 | 适用场景 |
| --- | --- | --- |
| **桌面快捷方式** | 双击 `tools\create_shortcut.vbs` 生成「总控台」快捷方式（带品牌图标），之后双击快捷方式启动 | **推荐**。无窗口、无命令行，体验等同 macOS 的 `.app` |
| 双击脚本 | 双击 `start.bat` | 备用。**后台运行**（pythonw 无窗口），自动安装依赖并启动，浏览器自动打开；已运行时直接打开浏览器 |
