...
- **卡片**：大按钮启动/停止（任务是运行/中止）；右侧一排小按钮（打开页面/复制链接/日志/诊断/重启/编辑/删除）常显，不用悬浮。运行中显示端口与时长；配置失效（目录/脚本丢失）会直接标出原因并禁用启动，点开「启动诊断」有修复建议。
...
- 先看左侧 **启动台**，不要在 **服务监控** 里找已停止的卡片。
- 没删过却变空：配置一般还在 `%APPDATA%\总控台\config.json`。先刷新启动台；仍为空则托盘「停止总控台」后再双击 `LocalOpsConsole.exe`。启动器会清理残留/异常进程，并把磁盘上的卡片读回内存（不会用空缓存把配置写丢）。
- 不要同时开 exe、`start.bat` 和 `python server.py`。
