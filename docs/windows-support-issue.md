...
1. **进程身份与本地控制（已修复）**：Windows 现在从进程 TokenUser 读取 SID，未知身份不会被视为当前用户；所有写接口还要求私有 `control.token` 对应的能力令牌。启动器以 URL fragment 传令牌，前端清理地址栏后仅在当前标签页保存。
...
2. **控制令牌维护**：后续改动不得恢复无令牌的本机写接口，也不得把 `control.token` 写入日志、命令历史或 URL query。
