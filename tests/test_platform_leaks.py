...
双向验证：
1. 基线：当前代码库必须 0 泄漏；
2. 反向：工具必须能检出真实泄漏，并跳过 docstring / 注释。
"""
...
class PlatformLeakScannerTests(unittest.TestCase):
    ...
    def test_detects_posix_branch_body(self):
        src = ('import sysops\n'
               'if sysops.IS_POSIX:\n'
    ...
    def test_detects_windows_branch_else(self):
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            hits = cpl.scan_py(path, cpl.BACKEND_PATTERNS)
        labels = [label for _, label, _ in hits]
        self.assertIn("osascript 命令", labels)

    def test_detects_implicit_else_tail(self):
    ...
    def test_detects_mod_key_command_symbol(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            hits = cpl.scan_frontend(path, cpl.FRONTEND_PATTERNS)
        labels = [label for _, label, _ in hits]
        self.assertIn("macOS 命令符号", labels)
